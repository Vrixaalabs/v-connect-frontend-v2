...
import { Building2, Users, School, Activity, TrendingUp, UserPlus, Bell, Shield, AlertCircle } from 'lucide-react';
...
const SuperAdminDashboard = () => {
  ...
  const {
    ...
    error: statsError,
  } = useQuery(GET_SUPER_ADMIN_DASHBOARD_STATS, {
    onError: error => {
  ...
  } = useQuery(GET_RECENT_ACTIVITIES, {
    variables: { limit: 5 },
    onError: error => {
  ...
  const recentActivities =
    activitiesData?.getRecentActivities.map((activity: Activity) => ({
      type: activity.type,
        ...
        <MotionCard className='col-span-1'>
          ...
          <div className='space-y-4'>
            {recentActivities.map(
              (activity: { type: string; message: string; time: string; icon: any }, index: number) => (
                <motion.div
                  key={index}
          ...
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              ...
              <Badge
                ...
                className={cn(
                  'bg-opacity-10',
                  systemData?.getSystemStatus.status === 'operational'
