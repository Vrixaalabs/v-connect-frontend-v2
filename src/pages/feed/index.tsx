...
const categories = ['Academic', 'Sports', 'Cultural', 'Technical', 'Social', 'Others'];
...
const FeedPage = () => {
  ...
  const togglePostType = (type: string) => {
    setSelectedPostTypes(prev => (prev.includes(type) ? prev.filter(t => t !== type) : [...prev, type]));
  };
            ...
            <Sheet open={isFiltersOpen} onOpenChange={setIsFiltersOpen}>
              ...
              <SheetContent>
                <SheetHeader>
                  ...
                  <SheetDescription>Customize your feed by applying filters.</SheetDescription>
                </SheetHeader>
            ...
            <Dialog open={isCreatePostOpen} onOpenChange={setIsCreatePostOpen}>
              ...
              <DialogContent className='sm:max-w-[525px]'>
                <DialogHeader>
                  ...
                  <DialogDescription>Share updates, news, or announcements with your network.</DialogDescription>
                </DialogHeader>
        ...
        <div className='grid gap-4'>
          {filteredPosts.length === 0 ? (
            ...
            filteredPosts.map(post => (
              <div key={post.id} className='rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden'>
                {/* Post Header */}
                <div className='p-6 space-y-4'>
                  <div className='flex items-center justify-between'>
                    <div className='flex items-center gap-4'>
                      <div className='h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center'>
                        <span className='text-sm font-medium'>{post.author.name.charAt(0).toUpperCase()}</span>
                      </div>
                      <div>
                        ...
                        <div className='text-sm text-muted-foreground'>{new Date(post.timestamp).toLocaleString()}</div>
                      </div>
                    </div>
                    {post.priority && (
                  ...
                  {/* Post Image */}
                  {post.image && (
                    <img src={post.image} alt='Post attachment' className='rounded-lg w-full h-48 object-cover' />
                  )}
                ...
                <div className='border-t px-6 py-3 flex justify-between items-center bg-muted/50'>
                  ...
                  {post.type === 'event' && <Button size='sm'>Register</Button>}
                </div>
              </div>
            ))
          )}
